...
make_EHelper(add) {
  ...
  rtl_is_add_overflow(&t0,&s0,&id_dest->val,&id_src->val,id_dest->width);
  rtl_set_OF(&t0);
  rtl_is_add_carry(&t0,&s0,&id_dest->val);
  rtl_set_CF(&t0);
...
make_EHelper(sub) {
 ...
 // printf("??%x\n",id_src->val);
  ...
  operand_write(id_dest, &s0);
  rtl_update_ZFSF(&s0,id_dest->width);
  rtl_is_sub_overflow(&t0,&s0,&id_dest->val,&id_src->val,id_dest->width);
  rtl_set_OF(&t0);
  rtl_is_sub_carry(&t0,&s0,&id_dest->val);
  rtl_set_CF(&t0);
