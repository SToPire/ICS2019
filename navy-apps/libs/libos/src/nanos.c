...
int _write(int fd, void *buf, size_t count) {
  int ret=(int)_syscall_(SYS_write,fd, (intptr_t)buf, count);
  return ret;
}
extern char _end;
static intptr_t pbrk=(intptr_t)&_end;	
void *_sbrk(intptr_t increment) {
  intptr_t obrk=pbrk;
  intptr_t nbrk=obrk+increment;
  int ret1=_syscall_(SYS_brk,nbrk,0,0);
  if(ret1==0) 
  {
    pbrk=nbrk;
    return (void*)obrk;
...
size_t _read(int fd, void *buf, size_t count) {
  size_t ret=(size_t)_syscall_(SYS_read, fd, (intptr_t)buf, count);   
  return ret;
}
